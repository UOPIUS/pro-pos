...
$adjustment_active = $role_has_permissions_list->where('name', 'adjustment')->first();
            ?>
    @if(
    $category_permission_active || $index_permission_active || $print_barcode_active || $stock_count_active
    || $adjustment_active
    )
...
$sale_add_permission_active = $role_has_permissions_list->where('name', 'sales-add')->first();
            ?>
    @if(
    $sale_index_permission_active || $packing_slip_challan_active || $gift_card_permission_active ||
    $coupon_permission_active || $delivery_permission_active
    )
    <li><a href="#sale" aria-expanded="false" data-toggle="collapse"> <i
        ...
        <ul id="sale" class="collapse list-unstyled ">
            ...
            <li id="challan-list-menu"><a href="{{route('challan.index')}}">{{trans('file.Challan List')}}</a>
            </li>
            @endif
    ...
    <li>
        ...
        <ul id="manufacturing" class="collapse list-unstyled ">
            <li id="production-list-menu"><a href="{{route('productions.index')}}">{{trans('file.Production List')}}</a>
            </li>
            <li id="production-create-menu"><a
                    href="{{route('productions.create')}}">{{trans('file.Add Production')}}</a>
            </li>
        </ul>
    </li>
    @endif
...
$account_statement_permission_active = $role_has_permissions_list->where('name', 'account-statement')->first();
            ...
            ?>
    @if(
    $index_permission_active || $balance_sheet_permission_active || $account_statement_permission_active ||
...
/**
 * Previous condition to show HRM module.
 * $department_active || $index_employee_active || $attendance_active || $payroll_active || $holiday_active
 */
            ?>

    @if($department_active == "Show")
...
$supplier_index_permission_active = $role_has_permissions_list->where('name', 'suppliers-index')->first();
            ...
            ?>
    @if(
    $user_index_permission_active || $customer_index_permission_active || $biller_index_permission_active ||
...
$supplier_due_report_active = $role_has_permissions_list->where('name', 'supplier-due-report')->first();
            ...
            ?>
    @if(
    $profit_loss_active || $best_seller_active || $warehouse_report_active || $warehouse_stock_report_active
    ...
    || $user_report_active || $biller_report_active || $customer_report_active || $supplier_report_active ||
    ...
    <li><a href="#report" aria-expanded="false" data-toggle="collapse"> <i
        ...
        <ul id="report" class="collapse list-unstyled ">
            ...
            <li id="due-report-menu">
                {!! Form::open([
                'route' => 'report.customerDueByDate',
                'method' => 'post',
                'id' =>
            ...
            <li id="supplier-due-report-menu">
                {!! Form::open([
                'route' => 'report.supplierDueByDate',
                'method' => 'post',
                'id' =>
    ...
    @endif
    @if(\Auth::user()->role_id == "set-to-5-to-show-this-menu")
    ...
    <li><a href="{{route('woocommerce.index')}}"> <i class="fa fa-wordpress"></i><span>WooCommerce</span></a>
    </li>
    @endif
...
$custom_field_permission_active = $role_has_permissions_list->where('name', 'custom_field')->first();
                    ?>
            @if($role->id <= 2) <li id="role-menu"><a
    ...
    <li id="custom-field-list-menu"><a href="{{route('custom-fields.index')}}">{{trans('file.Custom Field List')}}</a>
    </li>
    @endif
    @endif
    ...
    @endif
    <li id="payment-gateway-setting-menu"><a href="{{route('setting.gateway')}}">{{trans('file.Payment Gateways')}}</a>
    </li>
    ...
    @endif
    <li id="barcode-setting-menu"><a href="{{route('barcodes.index')}}"> {{trans('file.Barcode Settings')}}</a>
    </li>
